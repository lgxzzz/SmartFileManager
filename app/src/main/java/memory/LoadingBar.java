...
//                    Log.e("WechatApi","handler thread id = "+Thread.currentThread().getId());
        ...
        mRunnableFailed = new Runnable() {
            ...
            public void run() {
                if (pathX < minSide - mCenterX / 3 - padding) {
                	...
                	mHandlerFailed.postDelayed(new Runnable() {
        ...
        mRunnableSuccess = new Runnable() {
            ...
            public void run() {
                if (pathX < mCenterX - 10) {
                    ...
                    mHandlerSuccess.postDelayed(new Runnable() {
...
//        loading();
    ...
    ILoadingFinishListener listener;

    /**
     * @param result
     */
    public void loadingComplete(boolean result, ILoadingFinishListener listener) {
        this.listener = listener;
//        isLoading = false;
        isRunnableLoad = false;
        if (result) {
            success();
        } else {
            failed();
        ...
        //Test,add
        mHandlerLoading.removeCallbacks(mRunnableLoading);
    }
